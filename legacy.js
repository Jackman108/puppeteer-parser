...
(async () => {
    try {
        ...
        let allSubmittedFormsIndexes = new Set(); // Хранилище индексов успешно отправленных форм


        // Функция для обработки вакансии
        const processVacancy = async (vacancyIndex, vacancyElementsSelector, page) => {
            ...
            // Проверяем, содержит ли кнопка текст "Откликнуться" и не была ли вакансия уже обработана
            if (text.includes('Откликнуться') && !allSubmittedFormsIndexes.has(vacancyIndex)) {
                ...
                allSubmittedFormsIndexes.add(vacancyIndex);
            }
        };
        ...
        while (pages > idx) {
            try {
                ...
                if (nextPageButtonHandle) {
                    ...
                    break; // Завершаем цикл, если больше нет страниц
                }
            } catch (err) {
